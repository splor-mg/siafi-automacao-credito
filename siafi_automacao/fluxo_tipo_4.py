...
def tipo_4(em, data_row, uo_anterior, orientacao_anterior, linha, conclusao):
    ...
    while True:
        ...
        if data_row['orientacao'] == 'Suplementar':
            ...
            em.fill_field(linha, 47, '1', 1) #indica que é remanejamento com dotação propria
            ...
            em.fill_field(linha, 5, data_row['funcao'], 2) # função
            em.fill_field(linha, 8, data_row['subfuncao'], 3) # subfunção
            em.fill_field(linha, 12, data_row['programa'], 3) # programa
            em.fill_field(linha, 16, data_row['acao'], 4) # ação
            em.fill_field(linha, 21, '0001', 4)
            em.fill_field(linha, 28, f"{data_row['categoria']}{data_row['grupo']}{data_row['modalidade']}{data_row['elemento']}", 6) # natureza de despesa
            em.fill_field(linha, 39, data_row['iag'], 1) # procedência
            em.fill_field(linha, 44, data_row['fonte'], 2) # fonte
            em.fill_field(linha, 47, data_row['procedencia'], 1) # procedência

            if data_row['procedencia'] == '2':
                em.fill_field(linha, 53, data_row['uo_suplementada'], 4) # uo_financiadora
            em.fill_field(linha, 62, data_row['valor'], 15) # valor
