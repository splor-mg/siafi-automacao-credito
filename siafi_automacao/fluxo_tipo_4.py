    ...
    while True:  
        ...
        if uo_anterior != data_row['uo']:
            ...
            if data_row['orientacao'] == 'Suplementar':
                ...
                em.fill_field(14, 8, 'x', 1)
                em.send_enter()
                em.wait_for_field()

                em.fill_field(10, 67, data_row['uo'], 4)
                em.send_enter()
                em.wait_for_field()
        ...
        if data_row['orientacao'] == 'Suplementar':
            ...
            em.fill_field(linha, 47, '2', 1) #indica que é remanejamento com dotação propria
            ...
            em.fill_field(linha, 14, data_row['acao'], 4) # ação
            em.fill_field(linha, 19, '0001', 4)
            em.fill_field(linha, 26, f"{data_row['categoria']}{data_row['grupo']}{data_row['modalidade']}{data_row['elemento']}", 6) # natureza de despesa
            em.fill_field(linha, 37, data_row['iag'], 1) # procedência
            em.fill_field(linha, 42, data_row['fonte'], 2) # fonte
            em.fill_field(linha, 45, data_row['procedencia'], 1) # procedência

            if data_row['procedencia'] == '2':
                em.fill_field(linha, 52, data_row['uo_suplementada'], 4) # uo_financiadora
